        ...
        *   test_y:
        ...
        try:
            ...
            if len(test_y.unique()) == 1:  # if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.logger.info('Accuracy for RandomForest:' + str(self.random_forest_score))
            else:
                self.random_forest_score = roc_auc_score(test_y, self.prediction_random_forest)  # AUC for XGBoost
                self.logger.info('AUC for RandomForest:' + str(self.random_forest_score))

            #comparing the two models
            self.logger.info('End of finding best model...')
            if(self.random_forest_score <  self.xgboost_score):
                self.logger.info('XGBoost is the best model')
                return 'XGBoost',self.xgboost
            else:
                self.logger.info('RandomForest is the best model')
